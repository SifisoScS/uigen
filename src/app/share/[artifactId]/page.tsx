...
import { Package, Tag, GitBranch, Calendar, User, GitFork, Workflow } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-neutral-100">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        {/* Cross-artifact parents (WorkflowRun etc.) */}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {crossParents.map((cp) => (
                <a
                  key={cp.id}
                  href={`/workflow-run/${cp.id}`}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-amber-800/40 bg-amber-950/30 text-amber-400 text-xs hover:border-amber-700/60 hover:bg-amber-950/50 transition-colors"
                >
                  <Workflow className="h-3 w-3" />
                  <span className="font-medium">{cp.parentName}</span>
                  <span className="text-amber-700 text-[10px]">{cp.relationType}</span>
                </a>
              ))}
            </div>
          </div>
