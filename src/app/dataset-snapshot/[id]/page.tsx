...
import { getDatasetSnapshotDetail } from "@/actions/get-dataset-snapshot-detail";
...
export default async function DatasetSnapshotPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const data = await getDatasetSnapshotDetail(id);
  if (!data) notFound();

  const { snapshot, artifacts, govEvents } = data;
  const backHref =
    artifacts.length > 0 ? `/lineage/${artifacts[0].id}` : "/";

  const columnSummary = snapshot.columnSummary as {
    columns?: string[];
    types?: string[];
  } | null;
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-neutral-100">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-bold text-neutral-50">{snapshot.name}</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-1">
            ...
            {snapshot.format && (
              <span className="px-2 py-0.5 rounded-full bg-emerald-950/40 border border-emerald-800/40 text-emerald-400 text-xs font-medium uppercase">
                {snapshot.format}
              </span>
            )}
            {snapshot.rowCount != null && (
              <span className="px-2 py-0.5 rounded-full bg-neutral-900 border border-neutral-800 text-neutral-400 text-xs font-mono">
        ...
        {/* Column summary */}
        {columnSummary?.columns && columnSummary.columns.length > 0 && (
          <section aria-label="Columns">
            ...
            <div className="rounded-lg border border-[#2a2a2a] bg-[#0c0c0c] overflow-hidden">
              <div className="divide-y divide-[#1f1f1f]">
                {columnSummary.columns.map((col, i) => (
                  <div
                    key={col}
                    className="flex items-center justify-between px-4 py-2"
                  >
                    <span className="text-xs font-mono text-neutral-300">{col}</span>
                    {columnSummary.types?.[i] && (
        ...
        <section aria-label="Related UI Artifacts">
          ...
          {artifacts.length === 0 ? (
            <p className="text-xs text-neutral-600">
              No artifacts linked to this dataset snapshot yet.
            </p>
          ) : (
            <div className="space-y-2">
              {artifacts.map((a) => {
